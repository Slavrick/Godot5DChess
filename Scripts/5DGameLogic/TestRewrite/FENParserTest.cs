...
namespace TestRewrite
{
	...
	public static class FENParserTest
	{
		public static void TestMoveParser()
		{
			...
			CoordFive x = FENParserRewrite.StringToCoord("(0,0,0,0)");
			if (!new CoordFive(0, 0, 0, 0).Equals(x)) throw new Exception(x.ToString());
			x = FENParserRewrite.StringToCoord("(10,5,10,5)");
			if (!new CoordFive(10, 5, 10, 5).Equals(x)) throw new Exception(x.ToString());
			x = FENParserRewrite.StringToCoord("(-10,-5,-2,-1)");
		...
		public static void TestShadFEN()
		{
			...
			FENParserRewrite.ShadSTDGSM("res://PGN/Puzzles/Brawn Tactics 1.5DPGN.txt");
			FENParserRewrite.ShadSTDGSM("res://PGN/testPGNs/AmbiguityCheck.txt");
			Console.WriteLine(" passed.");
		}
